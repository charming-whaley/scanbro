...
public struct InfiniteScrollView<Content>: View where Content: View {
            ...
            ScrollView(.horizontal) {
                HStack(spacing: spacing) {
                    Group(subviews: content) { collection in
                        ...
                        let repeating = contentSize.width > 0 ? Int((size.width / averageWidth).rounded()) + 1 : 1
                        
                        HStack(spacing: spacing) {
                            ForEach(0..<repeating, id: \.self) { index in
                ...
                .background(
                    InfiniteScrollHelper(contentSize: $contentSize, decelerationRate: .constant(.fast))
                )
            }
        }
    }
    ...
    fileprivate struct InfiniteScrollHelper: UIViewRepresentable {
        ...
        func makeCoordinator() -> Coordinator {
            Coordinator(with: decelerationRate, of: contentSize)
        }
        ...
        final class Coordinator: NSObject, UIScrollViewDelegate {
            ...
            init(with decelerationRate: UIScrollView.DecelerationRate, of contentSize: CGSize) {
